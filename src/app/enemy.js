...
export default class extends Entity {
  constructor(opts = {}) {
    ...
    switch(t._subType) {
      ...
      case 'launcher':
        t._p.x      = w._viewport_bounds.spawnPoints[t._spawn].x;
        t._p.y      = w._viewport_bounds.spawnPoints[t._spawn].y;
  ...
  update(ctx) {
    ...
    let vNorm = t => {
      t._v.r += Math.cos(t._p.angle) / 10; 
      t._v.m += -Math.sin(t._p.angle) / 10;
      t._p.x += t._v.r * t._speed;
      t._p.y -= t._v.m * t._speed;
    }

    switch(this._subType) {
      case 'floater':
        if(w.oneIn(100)) {
          this._p.angle = getAngle(this._p, w._viewport_bounds.ranPos()) + w.ranRad();          
        }
        vNorm(this); 
        if(this._bounce()) {
          this._p.angle = getAngle(this._p, w._viewport_bounds.ranPos());
        }           
        break;
